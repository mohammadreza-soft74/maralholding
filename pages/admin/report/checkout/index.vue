...
export default {
  ...
  async asyncData({$axios}) {
    ...
    let entries = await $axios.get('api/report/searchcheckoutbydate');
  ...
  methods: {
    async rangeSearch() {
      this.proccess = true;
      let response = await this.$axios.get('api/report/searchcheckoutbydate', {
        params: {
          from: this.year.from,
          to: this.year.to
        }
    ...
    async changePage() {
      let response = await this.$axios.get('api/report/searchcheckoutbydate?page=' + this.entries.meta.current_page)
      this.entries = response.data
    },
